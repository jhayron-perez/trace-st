...
warnings.filterwarnings(
    "ignore",
    message="In analysis mode 'single'",
...
def concatenate_timeseries_nonwrapping(
    data,
    y_ascending,
    include_cell_index_column=False,
    radius=1,
):
    """
    Concatenate timeseries from an extended Moore neighborhood (non-wrapping).

    Parameters
    ----------
    data : np.ndarray
        Array of shape (ny, nx, time).
    y_ascending : bool
        If True: latitude increases with row index (data[0,:,:] is south, data[-1,:,:] is north).
        If False: latitude decreases with row index (data[0,:,:] is north, data[-1,:,:] is south).
        In all cases, the neighborhood ordering is such that neighbor position 0
        corresponds to the north-west (most poleward and western) pixel.
    include_cell_index_column : bool
        If True, append a column with the linear cell index.
    radius : int
        Neighborhood radius in grid points. radius=1 → 3x3, radius=2 → 5x5, etc.

    Returns
    -------
    np.ndarray
        2D array: (num_cells * time, num_neighbors [+1 if index column]).
    """
    ROW_LEN, COL_LEN, T = data.shape

    # interior cells with full neighborhood
    ROW_RANGE = np.arange(radius, ROW_LEN - radius)
    if y_ascending:
        # iterate from north to south in physical space
        ROW_RANGE = ROW_RANGE[::-1]

    COL_RANGE = np.arange(radius, COL_LEN - radius)

    num_neighbors = (2 * radius + 1) ** 2

    if include_cell_index_column:
        concatenated_data = [[] for _ in range(num_neighbors + 1)]
        index = 0
    else:
        concatenated_data = [[] for _ in range(num_neighbors)]
        ...
        for col in COL_RANGE:
            ...
            for di in range(-radius, radius + 1):
                for dj in range(-radius, radius + 1):
                    ...
                    # Ensure di < 0 is always "north", independent of y ordering
                    if y_ascending:
                        rr = row - di
                    else:
                        rr = row + di
                    ...
                    concatenated_data[pos].extend(data[rr, cc, :])
                    pos += 1

            if include_cell_index_column:
                concatenated_data[-1].extend([index] * T)
                index += 1

    concatenated_data = np.array(concatenated_data).T
    return concatenated_data
...
def get_MV_reduced_space(
    ...
    ----------
    data : np.ndarray
        A 4D numpy array of shape (variable_n, Y, X, T) representing the data for multiple variables
        across a 2D spatial grid over time.
    dependencies_wrap : bool
        If True, wrap dependencies at the edges of the spatial grid during preprocessing.
    y_ascending : bool
        If True, latitude increases with row index (0 = south, last = north).
        If False, latitude decreases with row index (0 = north, last = south).
    radius : int
        Neighborhood radius in grid points. radius=1 → 3x3, radius=2 → 5x5, etc.
    ...
    -------
    np.ndarray
        2D array representing the reduced-space concatenated time series for each variable.
        Shape: (num_cells * T, variable_n * neighborhood_size).
    """

    var_n = data.shape[0]
    reduced_data: list[np.ndarray] = []

    for variable in range(var_n):
        var_data = data[variable, :, :, :]  # shape (Y, X, T)

        if dependencies_wrap:
            # assumes you update this function to same API as nonwrapping
        ...
        reduced_data.append(concatenated_data)

    # Combine reduced spaces along feature dimension
...
def build_link_assumptions(
    ...
    child_variable: if not None, only one possible child is studied, the variables labeling starts in 1
    Returns
    -------
    dict
    ...
    stencil_size = (2*radius + 1)**2
    center_index = stencil_size // 2
    possible_children = list(range(center_index, node_count, stencil_size))
    
    if child_variable:
        possible_children = [possible_children[child_variable-1]]
    # print(possible_children)
    link_assumptions: Dict[int, Dict[Tuple[int, int], str]] = {} # for each child, it contains a tuple with parent and tau, and the prescribed assumption
            ...
            for parent_i in range(node_count):
                for lag in range(min_tau, max_tau + 1):
                    if intervariable_link_assumptions is not None:
                        ...
                        child_intervar_link_idx = (child_j - center_index) // stencil_size
                        parent_intervar_link_idx = parent_i // stencil_size
                        try:
...
def get_graph_from_pmatrix_safe(pcmci, p_matrix, alpha_level, tau_min, tau_max, link_assumptions=None):
    """
    Standalone safe wrapper around Tigramite's get_graph_from_pmatrix that fixes:
      - the '==' bug (should be assignment)
      - optionally avoids mutating caller p_matrix
    Keeps Tigramite's graph formatting via pcmci.convert_to_string_graph.
    """
    _int_link_assumptions = pcmci._set_link_assumptions(link_assumptions, tau_min, tau_max)

    if link_assumptions is not None:
        mask = np.zeros((pcmci.N, pcmci.N, tau_max + 1), dtype=bool)
        for j, links_ in _int_link_assumptions.items():
            for (i, lag), ltype in links_.items():
                if ltype not in ["<--", "<?-"]:
                    mask[i, j, abs(lag)] = True
    else:
        mask = np.ones((pcmci.N, pcmci.N, tau_max + 1), dtype=bool)

    pm = p_matrix.copy()     # avoid mutating caller; remove this line if you truly want in-place
    pm[~mask] = 1.0          # <-- THE BUG FIX (assignment, not comparison)

    graph_bool = pm <= alpha_level
    graph = pcmci.convert_to_string_graph(graph_bool)
    return graph
...
def mv_CaStLe_PC(
    ...
    return_reduced_space: bool = False,
    verbose: int = 1,
    child_variable: int = None
) -> Tuple:
    """
    Executes the multivariate CaStLe-PC/MCI algorithm on space-time data to infer causal relationships.

    This version is compatible with an extended Moore neighborhood of radius `radius`,
    so each grid cell is represented by a stencil of size:
        stencil_size = (2*radius + 1)**2
    ...
    y_ascending : bool, optional
        If True, latitude increases with row index (data[0,:,:] is south, data[-1,:,:] is north).
        If False, latitude decreases with row index (data[0,:,:] is north, data[-1,:,:] is south).
        Used by the neighborhood concatenation to ensure a consistent "north-west first" ordering.
    radius : int, optional
        Neighborhood radius in grid points. radius=1 → 3x3 stencil (size 9),
        radius=2 → 5x5 stencil (size 25), etc.
    ...
    graph_p_threshold : float
        Threshold for considering dependencies as significant when reconstructing the causal graph.
    min_tau : int, optional
        Minimum lag to consider. Must be 0 or 1. Defaults to 1.
    cd_function : str, optional
        "run_pcalg" or "run_pcmci". Defaults to "run_pcalg".
    fdr_method : str, optional
        FDR correction method (e.g., "bh"). Defaults to None.
    intervariable_link_assumptions : dict, optional
        Variable-level link assumptions {j: {(i, -tau): link_type, ...}, ...}.
        If None, a naive stencil is constructed.
    allow_center_directed_links : bool, optional
        If False, links between "center" nodes (one per grid cell/variable) are undirected ("o?o").
        If True, directed links are allowed.
    child_variable: if not None, only one possible child is studied, the variables labeling starts in 1
    
    Post-Processing Parameters:
    strength_threshold : float, optional
        Threshold for filtering edges based on strength.

    Output Parameters:
    return_reduced_space : bool, optional
        If True, also return the reduced space.
    ...
    -------
    tuple
        Same structure as in the original mv_CaStLe_PC, plus optionally `reduced_space`
        if return_reduced_space=True.
    """
    assert len(data.shape) == 4, "data needs to have 4 dimensions (variable_n, Y, X, T)"
    assert graph_p_threshold < 1.0, "graph_p_threshold must be less than 1.0, otherwise illegal links are output."

    # For now still using a fixed max_tau = 1 as in the original
    max_tau = 1

    # --- NEW: pass y_ascending and radius into the reduced-space builder ---
    ...
    if verbose:
        print(f"Data concatenated into extended Moore neighborhood space (shape {reduced_space.shape})...")

    # # Number of nodes in reduced space
    node_count = reduced_space.shape[1]

    # Build link assumptions consistent with the new stencil layout
    ...
    # print("reduced_space shape:", reduced_space.shape)
    # print("original T:", data.shape[-1])
    # print("node_count:", reduced_space.shape[1])
    # print("reduced_space rows / T =", reduced_space.shape[0] / data.shape[-1])
    ...
    if fdr_method:
        if fdr_method == "bh":
            fdr_method = "fdr_bh"  # adapt to Tigramite naming
        ...
        results["graph"] = get_graph_from_pmatrix_safe(
            pcmci, q_matrix, graph_p_threshold, min_tau, max_tau, link_assumptions=link_assumptions
        )
        results["q_matrix"] = q_matrix
    else:
        results["graph"] = get_graph_from_pmatrix_safe(
            pcmci, results["p_matrix"], graph_p_threshold, min_tau, max_tau, link_assumptions=link_assumptions
        )

    # Optional strength thresholding
...
def mv_CaStLe_DYNOTEARS(
    data: np.ndarray,
    *,
    # --- keep API close to mv_CaStLe_PC ---
    pc_alpha: float = 0.2,                 # unused (DYNOTEARS doesn't use p-values)
    graph_p_threshold: float = 0.001,      # unused
    min_tau: int = 1,                      # DYNOTEARS uses p=max_tau; we enforce max_tau=1; min_tau must be 1
    strength_threshold: Optional[float] = None,  # optional post-threshold on |weights|
    y_ascending: bool = False,
    radius: int = 1,
    fdr_method: Optional[str] = None,      # unused
    intervariable_link_assumptions: Optional[dict] = None,  # not supported in DYNOTEARS version below
    allow_center_directed_links: bool = False,
    dependencies_wrap: bool = False,
    return_reduced_space: bool = False,
    verbose: int = 1,
    child_variable: Optional[int] = None,
    # --- DYNOTEARS-specific knobs ---
    lambda_w: Optional[float] = None,
    lambda_a: float = 0.01,
    max_iter: int = 200,
    dependence_threshold: float = 0.01,    # passed to w_threshold
) -> Tuple[Dict[str, Any], Optional[np.ndarray]]:
    """
    DYNOTEARS-backed CaStLe (multivariate stencil) with an API similar to mv_CaStLe_PC.

    Returns
    -------
    results : dict
        Contains at least:
          - "graph": np.ndarray of shape (node_count, node_count, 2) with "" or "-->"
          - "val_matrix": np.ndarray same shape with edge weights
        and a few extra metadata fields.
    reduced_space : np.ndarray (optional)
        Only returned when return_reduced_space=True
    """
    if from_pandas_dynamic is None:
        raise ModuleNotFoundError(
            "causalnex is required for DYNOTEARS. Install causalnex to use mv_CaStLe_DYNOTEARS."
        )

    assert data.ndim == 4, "data must have 4 dimensions (variable_n, Y, X, T)"
    assert min_tau in (1,), "This DYNOTEARS wrapper currently supports min_tau=1 only."
    if intervariable_link_assumptions is not None:
        raise NotImplementedError(
            "intervariable_link_assumptions is not implemented for DYNOTEARS in this wrapper yet."
        )

    if lambda_w is None:
        lambda_w = lambda_a

    max_tau = 1  # keep consistent with your PCMCI setup

    # --- Build reduced space in the same way as PCMCI version ---
    reduced_space = get_MV_reduced_space(
        data=data,
        dependencies_wrap=dependencies_wrap,
        y_ascending=y_ascending,
    ...
    if verbose:
        print(f"[DYNOTEARS] reduced_space shape = {reduced_space.shape} (node_count={node_count})")

    # --- Determine center nodes (children-eligible nodes) exactly like build_link_assumptions ---
    # In the reduced space, nodes are grouped by variable (each group = stencil_size neighbors)
    # Centers are at: center_index + k*stencil_size
    possible_children = list(range(center_index, node_count, stencil_size))

    if child_variable is not None:
        if not (1 <= int(child_variable) <= len(possible_children)):
            raise ValueError(
                f"child_variable={child_variable} out of bounds. Must be 1..{len(possible_children)}"
            )
        possible_children = [possible_children[int(child_variable) - 1]]

    center_nodes = possible_children
    taboo_children = [str(i) for i in range(node_count) if i not in center_nodes]

    # --- Build tabu edges (block illegal edges) ---
    col_names = [str(i) for i in range(node_count)]
    tabu_edges = []

    # Ban all lag-0 edges (same as your original DYNOTEARS function)
    for i in col_names:
        for j in col_names:
            tabu_edges.append((0, i, j))

    # Ban center-to-center directed links at lag-1 unless allowed
    if not allow_center_directed_links:
        for a in center_nodes:
            for b in center_nodes:
                if a != b:
                    tabu_edges.append((1, str(a), str(b)))

    # --- Prepare dataframe ---
    df_castled = pd.DataFrame(reduced_space, columns=col_names)

    if verbose:
        msg = (
            f"[DYNOTEARS] Running with:\n"
            f"  radius={radius} stencil_size={stencil_size}\n"
            f"  y_ascending={y_ascending} wrap={dependencies_wrap}\n"
            f"  center_nodes={center_nodes}\n"
            f"  tabu_edges={len(tabu_edges)} tabu_children={len(taboo_children)}\n"
            f"  lambda_a={lambda_a} lambda_w={lambda_w} max_iter={max_iter} w_threshold={dependence_threshold}\n"
        )
        print(msg)

        if fdr_method is not None or pc_alpha is not None or graph_p_threshold is not None:
            # They exist for API compatibility; DYNOTEARS doesn't use them.
            print("[DYNOTEARS] Note: pc_alpha / graph_p_threshold / fdr_method are ignored in DYNOTEARS mode.")
    ...
    structure_model = from_pandas_dynamic(
        ...
        w_threshold=float(dependence_threshold),
        tabu_edges=tabu_edges,
        tabu_child_nodes=taboo_children,
    )

    # --- Convert to Tigramite-style (graph,val_matrix) ---
    reconstructed_graph, val_matrix = get_graph_from_structure_model(structure_model)

    # Optional post-threshold by strength
    if strength_threshold is not None:
        mask = np.abs(val_matrix) < float(strength_threshold)
        val_matrix = val_matrix.copy()
        reconstructed_graph = reconstructed_graph.copy()
        val_matrix[mask] = 0.0
        reconstructed_graph[mask] = ""
    ...
    results: Dict[str, Any] = {
        ...
        "method": "dynotears",
        "radius": int(radius),
        "y_ascending": bool(y_ascending),
        "dependencies_wrap": bool(dependencies_wrap),
        "child_variable": int(child_variable) if child_variable is not None else None,
        "center_nodes": center_nodes,
        "dependence_threshold": float(dependence_threshold),
        "strength_threshold": float(strength_threshold) if strength_threshold is not None else None,
        "lambda_a": float(lambda_a),
        "lambda_w": float(lambda_w),
        "max_iter": int(max_iter),
    }
...
def _lasso_single_child_lag1(
    ...
    *,
    ...
    max_iter: int = 5000,
    l1_ratio: float | None = None,      # None => Lasso, else ElasticNet
    standardize: bool = True,
    refit_ridge: bool = True,
    ridge_alpha: float = 1e-2,
):
    """
    Fit sparse lag-1 parents for ONE child node using Lasso/ElasticNet,
    optionally refit Ridge on the selected support (post-lasso / debiasing).
    """
    import numpy as np

    # # reshape back into (n_cells, T, node_count)
    # n_rows = reduced_space.shape[0]
    # if n_rows % T != 0:
    #     raise ValueError(f"reduced_space rows ({n_rows}) not divisible by T ({T}).")
    # n_cells = n_rows // T
    # X3 = reduced_space.reshape(n_cells, T, node_count)

    # # lag-1 design
    # X = X3[:, :-1, :].reshape(-1, node_count).astype(float)
    # y = X3[:, 1:, child_node].reshape(-1).astype(float)

    ### CHATGPT WAY ###
    # reduced_space is (R*T, N) where rows are concatenated by cell then time
    n_rows = reduced_space.shape[0]
    if n_rows % T != 0:
        raise ValueError(f"reduced_space rows ({n_rows}) not divisible by T ({T}).")
    R = n_rows // T  # number of spatial samples (cells)
    
    Xall = reduced_space.astype(float)  # (R*T, N)
    
    # build valid (t-1 -> t) pairs within each cell block (prevents seam leakage)
    idx = np.arange(R * T).reshape(R, T)     # row indices laid out as (cell, time)
    src = idx[:, :-1].ravel()               # rows at t-1
    dst = idx[:,  1:].ravel()               # rows at t
    
    X = Xall[src, :]                        # predictors at t-1, shape ((R*(T-1)), N)
    y = Xall[dst, child_node]               # child at t, shape ((R*(T-1)),)
    ### CHATGPT WAY ###
    
    # print(X.shape)
    # print(y.shape)

    # --- optional standardization (helps Lasso/EN a lot) ---
    if standardize:
    ...
    sparse.fit(X, y)
    w = np.asarray(sparse.coef_, dtype=float)

    # --- support selection ---
    support = np.where(np.abs(w) >= float(dependence_threshold))[0]

    # --- optional debiasing: ridge on selected predictors ---
    if refit_ridge and support.size > 0:
        ...
        w_refit = np.zeros_like(w)
        w_refit[support] = np.asarray(ridge.coef_, dtype=float)
        w = w_refit

    # --- unstandardize coefficients back to original X scale ---
    if standardize:
        w = w / X_std  # undo X scaling

    # --- package into (node,node,2) outputs ---
    graph = np.empty((node_count, node_count, 2), dtype=object)
    graph[:] = ""
    val = np.zeros((node_count, node_count, 2), dtype=float)

    for j in range(node_count):
        if abs(w[j]) >= float(dependence_threshold):
            graph[j, child_node, 1] = "-->"
            val[j, child_node, 1] = w[j]

    return graph, val

    
def mv_CaStLe_GRANGER(
    data: np.ndarray,
    *,
    min_tau: int = 1,
    ...
    ridge_alpha: float = 1e-2,
    max_iter: int = 5000,
    dependence_threshold: float = 1e-4,
    strength_threshold: Optional[float] = None,
    fit_intercept: bool = True,          # ← ADD THIS
) -> Tuple[Dict[str, Any], Optional[np.ndarray]]:

    assert data.ndim == 4, "data must have 4 dimensions (variable_n, Y, X, T)"
    assert min_tau in (1,), "This GRANGER wrapper currently supports min_tau=1 only."
    ...
    # child candidates are center nodes for each variable block
    possible_children = list(range(center_index, node_count, stencil_size))

    if child_variable is not None:
        if not (1 <= int(child_variable) <= len(possible_children)):
            raise ValueError(f"child_variable={child_variable} out of bounds.")
        center_nodes = [possible_children[int(child_variable) - 1]]
    else:
        center_nodes = possible_children

    if verbose:
        print(f"[GRANGER] reduced_space shape={reduced_space.shape} node_count={node_count}")
        print(f"[GRANGER] center_nodes={center_nodes} lambda_a={lambda_a} dep_thr={dependence_threshold}")

    # infer T from original data (last dim)
    T = int(data.shape[-1])
    ...
    for child_node in center_nodes:
        g1, v1 = _lasso_single_child_lag1(
            reduced_space,
            node_count=node_count,
            ...
            refit_ridge=bool(refit_ridge),
            ridge_alpha=float(ridge_alpha),
        )
        # merge into full matrices
        graph[:, child_node, 1] = g1[:, child_node, 1]
        val[:, child_node, 1]   = v1[:, child_node, 1]

    # optional post-threshold
    if strength_threshold is not None:
        mask = np.abs(val) < float(strength_threshold)
        val = val.copy()
        graph = graph.copy()
        val[mask] = 0.0
        graph[mask] = ""
